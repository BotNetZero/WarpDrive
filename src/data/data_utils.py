...
class StreamDataset(IterableDataset):
    ...
    def get_sequence(self):
        ...
        for x in self.data:
            self.iter_count += 1
            curr_tokens = self.tokenizer.tokenize(self.doc_separator + x['text'])['input_ids'][0]
...
def get_train_data_loader(args, tokenizer, state_dict=None):
    ...
    for task in task_list:
        if ':' in task:
            file_name, prob = task.strip().split(':')
            task = os.path.join(DATA_PATH, file_name)
            prob = float(prob)
        else:
            task = task.strip()
            prob = 1.0
